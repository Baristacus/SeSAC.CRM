...
def get_users():
    ...
    if page < 1 or per_page * (page - 1) > User.query.count():
    ...
    if name:
        ...
        if user_total == 0:
            flash("해당 이름의 회원이 존재하지 않습니다.", "danger")
    ...
    if gender:
        ...
        if user_total == 0:
            flash("해당 성별의 회원이 존재하지 않습니다.", "danger")
            return redirect(url_for("user.get_users"))
    ...
    if age:
        ...
        if user_total == 0:
            flash("해당 나이의 회원이 존재하지 않습니다.", "danger")
            return redirect(url_for("user.get_users"))
...
def get_user(id):
    try:
        ...
        flash("사용자 정보를 가져오는 중 오류가 발생했습니다.", "danger")
        return redirect(url_for("user.get_users"))
